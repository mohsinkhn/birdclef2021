...
from src.lightningmodels import BirdModel2
...
def train_one_fold(df_tr, df_vl, config, stage, logdir, device, wandb_logger, resume_checkpoint):
    ...
    # Model
    model = BirdModel2(config, stage)
    checkpoint = ModelCheckpoint(
        dirpath=logdir, filename="{epoch}-{valid_f1:.3f}", monitor="test_f1", mode="max", verbose=True, period=2
    )
