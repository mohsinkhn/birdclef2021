...
def train_one_fold(df_tr, df_vl, config, stage, logdir, device, wandb_logger, resume_checkpoint):
    ...
    if (config.config.training.stage1.model.load_path != 'None'):
        model.load_state_dict(torch.load(config.config.training.stage1.model.load_path, map_location=device)['state_dict'], strict=False)

    checkpoint = ModelCheckpoint(
        dirpath=logdir, filename="{epoch}-{test_f1:.3f}", monitor="test_f1", mode="max", verbose=True, period=2
    )
